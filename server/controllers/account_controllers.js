  ...
  try {
    ...
    if (account.type === "credit card") {
      account.creditCard.number =
        creditCard.number || account.creditCard.number;
      account.creditCard.cvc = creditCard.cvc || account.creditCard.cvc;
      account.creditCard.expDate =
        creditCard.expDate || account.creditCard.expDate;
    }

    // Update bank account details if provided
    if (account.type === "bank account") {
      account.bankAccount.routingNumber =
        bankAccount.routingNumber || account.bankAccount.routingNumber;
      account.bankAccount.accountNumber =
        bankAccount.accountNumber || account.bankAccount.accountNumber;
    }
    await account.save();
    res.status(200).json(account);
  } catch (error) {
