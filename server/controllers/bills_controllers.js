// controllers/createBill_controller.js
...
const addBill = async (req, res) => {
  const { billName, amountDue, frequency, dueDate, nextDueDate, category } = req.body;
...
const updateBill = async (req, res) => {
  const { id } = req.params;
  const { billName, amountDue, frequency, dueDate, nextDueDate, category, status } = req.body;

  try {
    const updatedBill = await Bill.findOneAndUpdate(
      { _id: id, userId: req.user._id },
      { billName, amountDue, frequency, dueDate, nextDueDate, category, status },
      { new: true }
    );
...
const deleteBill = async (req, res) => {
  ...
  try {
    const deletedBill = await Bill.findOneAndDelete({ _id: id, userId: req.user._id });
